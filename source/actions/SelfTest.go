...
func SelfTest(console *structs.Console) bool {
	...
	if ebpf.ForbidAddress(console, "1.3.3.7") == false {
		console.Error("forbid address failed")
		result = false
	}

	if ebpf.IsForbiddenAddress(console, "1.3.3.7") == false {
		console.Error("is forbidden address failed")
		result = false
	}

	if ebpf.PermitAddress(console, "1.3.3.7") == false {
		console.Error("permit address failed")
		result = false
	}

	if ebpf.IsForbiddenAddress(console, "1.3.3.7") == true {
		console.Error("address still forbidden after permit")
		result = false
	}

	if ebpf.ForbidSubnet(console, "1.3.3.0", 24) == false {
		console.Error("forbid subnet failed")
		result = false
	}

	if ebpf.IsForbiddenSubnet(console, "1.3.3.7", 32) == false {
		console.Error("is forbidden subnet failed")
		result = false
	}

	if ebpf.PermitSubnet(console, "1.3.3.0", 24) == false {
		console.Error("permit subnet failed")
		result = false
	}

	if ebpf.IsForbiddenSubnet(console, "1.3.3.7", 32) == true {
		console.Error("subnet still forbidden after permit")
		result = false
	}

	if ebpf.ForbidPort(console, 1338) == false {
		console.Error("forbid port failed")
		result = false
	}

	if ebpf.IsForbiddenPort(console, 1338) == false {
		console.Error("is forbidden port failed")
		result = false
	}

	if ebpf.PermitPort(console, 1338) == false {
		console.Error("permit port failed")
		result = false
	}

	if ebpf.IsForbiddenPort(console, 1338) == true {
		console.Error("port still forbidden after permit")
		result = false
	}

	if ebpf.ForbidAddress(console, "evil.example") == false {
		console.Error("forbid domain failed")
		result = false
	}

	if ebpf.IsForbiddenAddress(console, "evil.example") == false {
		console.Error("is forbidden domain failed")
		result = false
	}

	if ebpf.PermitAddress(console, "evil.example") == false {
		console.Error("permit domain failed")
		result = false
	}

	if ebpf.IsForbiddenAddress(console, "evil.example") == true {
		console.Error("domain still forbidden after permit")
		result = false
	}

	attached := ebpf.AttachAll(console)
